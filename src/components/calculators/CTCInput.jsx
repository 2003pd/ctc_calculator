...
const CTCInput = ({
   ...
   const calculateSpecialAllowance = () => {
    if (inputMode === 'percentage') {
        const totalPct =
            Number(inputs.basic || 0) +
            Number(inputs.hra || 0) +
            Number(inputs.da || 0) +
            Number(inputs.empPF || 0) +
            (includeEmployerPF ? Number(inputs.emplrPF || 0) : 0) +
            (includeGratuity ? Number(inputs.gratuity || 0) : 0) +
            Number(inputs.other || 0) +
            Number(inputs.nps || 0);

        const specialPct = 100 - totalPct;
        return specialPct > 0 ? specialPct : 0;
    }
    ...
    const specialAmt = ctc - totalAmount;
    return specialAmt > 0 ? specialAmt : 0;
};
    ...
    value={
        inputMode === 'percentage'
            ? calculateSpecialAllowance().toFixed(2)
            : formatIndianNumber(calculateSpecialAllowance())
    }
    readOnly
